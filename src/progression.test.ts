...
describe('persistent achievements, missions and gem shop',()=>{
 ...
 it('gives new players only currently measurable missions',()=>{const s=rollPeriods(newGame(0),0);expect(s.missions.daily.tasks).toHaveLength(4);expect(s.missions.daily.tasks.some(t=>['researchStarted','researchCompleted','itemsCrafted','prestiges'].includes(t.metric))).toBe(false)});
