...
describe('phase 1 hardware and active play',()=>{
  ...
  it('uses additive permanent INT and fixed depth costs',()=>{const state={...newGame(0),totalINTEarned:12};expect(creditMultiplierFromINT(state)).toBeCloseTo(2.2);expect(prestigeUpgradeCost('dataArchive1',2)).toBe(1);});
  ...
  it('retires the removed intro analysis route',()=>{const s={...newGame(0),discovered:['calculator','sbc'] as ('calculator'|'sbc')[]};expect(queueExperiment(s,'hardware',0,'intro')).toBe(s);expect(s.experiments.active).toBeNull()});
