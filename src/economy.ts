...
export const BALANCE = {
  ...
  hardwareMilestones:[10,25,50], hardwareUnlockCount:10, classUpgradeCount:15, classUpgradeCostFactor:1,
  creditPerCompute: 1, qualityGrowth: 1.08, efficiencyGrowth: 1.04,
  trainingPerCompute: .12, trainingBase: 40, trainingGrowth: 1.65,
...
export type GameState = {
  credits:number; hardware:number; hardwareCounts:Record<HardwareId,number>; classUpgrades:HardwareId[]; discovered:HardwareId[]; level:number; training:number; savedAt:number; clockOffsetMs:number;
...
export const milestoneFactor=(count:number)=>2**BALANCE.hardwareMilestones.filter(n=>count>=n).length;
...
export const tapCredits=(s:GameState,now=s.savedAt)=>Math.max(1,BALANCE.tapRateFraction*creditRate(s.hardware,s.level,s,now,false));
