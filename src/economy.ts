...
export type GameState = {
  ...
  ads:{day:string;counts:Record<string,number>;transactions:string[]}; settings:{effects:boolean;buyMode:1|10|'max'}; testSave:boolean; nextId:number; telemetry:LocalTelemetry;
};
...
export function buyResearchProject(s:GameState,id:ResearchProjectId){const p=BALANCE.researchProjects[id];if(!p||s.completedResearch.includes(id)||s.credits<p.credits||s.data<p.data||s.researchPoints<p.points)return s;return addEvent({...s,credits:s.credits-p.credits,data:s.data-p.data,researchPoints:s.researchPoints-p.points,completedResearch:[...s.completedResearch,id]},'research-complete',s.savedAt,{id});}
