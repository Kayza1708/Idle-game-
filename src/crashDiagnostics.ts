...
export type CrashDiagnostics={version:1;sessionId:string;runId:string|null;sessionStartedAt:number;sessionEndedAt:number|null;cleanExit:boolean;lastUiHeartbeat:number|null;renderCount:number;visibility:{at:number;state:string}[];holds:Mark[];actions:Mark[];ticks:Mark[];saves:Mark[];failures:Failure[];longTasks:{at:number;durationMs:number}[];telemetry:{events:number;snapshots:number;saveBytes:number;storageSizes?:unknown;storageInventory?:unknown} | null;userAgent:string|null};
const cap=<T,>(items:T[],limit:number)=>items.slice(-limit);
const fallback=():CrashDiagnostics=>({version:1,sessionId:`session-${Date.now().toString(36)}`,runId:null,sessionStartedAt:Date.now(),sessionEndedAt:null,cleanExit:false,lastUiHeartbeat:null,renderCount:0,visibility:[],holds:[],actions:[],ticks:[],saves:[],failures:[],longTasks:[],telemetry:null,userAgent:typeof navigator==='undefined'?null:navigator.userAgent});
...
export class CrashRecorder{
 ...
 private mark(target:'actions'|'ticks'|'saves'|'holds',mark:Mark){this.state={...this.state,[target]:cap([...this.state[target],mark],target==='actions'?100:20)};this.write();}
 begin(target:'actions'|'ticks'|'saves'|'holds',name:string,runId=this.state.runId){const at=Date.now();if(target==='actions'&&name==='tap'&&at-this.lastTap<1000)return 0;if(name==='tap')this.lastTap=at;this.mark(target,{at,name,phase:'begin',runId:runId??undefined,sessionId:this.state.sessionId});return at;}
 end(target:'actions'|'ticks'|'saves'|'holds',name:string,started:number,result='ok',size?:number,durationMs?:number){if(!started)return;this.mark(target,{at:Date.now(),name,phase:'end',durationMs:durationMs??Math.max(0,Date.now()-started),result,size,runId:this.state.runId??undefined,sessionId:this.state.sessionId});}
 ...
 visibility(state:string){this.state={...this.state,visibility:cap([...this.state.visibility,{at:Date.now(),state}],20)};this.write()}
 counts(events:number,snapshots:number,saveBytes:number,storageSizes?:unknown,storageInventory?:unknown){this.state={...this.state,telemetry:{events,snapshots,saveBytes,storageSizes,storageInventory}};this.write()}
