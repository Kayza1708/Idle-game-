...
import { BALANCE, creditRate,computeRate,dataRate,newINT,researchRate } from './economy';
...
export function createBalanceExportFiles(state:GameState,exportedAt=Date.now()):Record<string,string>{
  ...
  const research=events.filter(x=>x.type==='research-start'||x.type==='research-complete').map(x=>[new Date(x.at).toISOString(),x.type,x.details.id??'',x.details.labId??'',x.details.duration??'',x.details.creditCost??'',x.details.dataCost??'',x.details.researchPointCost??'',x.details.materials??'',x.details.breakthrough??'']);
  ...
  const manifest={format:'ai-singularity-local-balance',exportVersion:EXPORT_VERSION,gameVersion:GAME_VERSION,saveVersion:SAVE_VERSION,createdAt:new Date(exportedAt).toISOString(),privacy:'local-only; no full save, player name, account, device fingerprint or automatic upload',limits:{recentEvents:500,snapshots:300}};
  const economy={researchProjects:BALANCE.researchProjects,researchDurationFormula:{baseSeconds:BALANCE.researchBaseSeconds,rankGrowth:BALANCE.researchRankGrowth,fixedAtStart:true},components:BALANCE.components,componentSources:BALANCE.componentSources,craftingRecipes:BALANCE.craftingRecipes};
  return{
    ...
    'research.csv':csv(['utc','event','project','labId','duration','creditCost','dataCost','researchPoints','materials','breakthrough'],research),
