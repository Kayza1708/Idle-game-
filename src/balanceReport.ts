...
export function createBalanceExportFiles(state:GameState,exportedAt=Date.now()):Record<string,string>{
  ...
  const crafting=events.filter(x=>x.type==='module-craft'||x.type==='item-craft'||x.type==='item-upgrade'||x.type==='item-equip').map(x=>[new Date(x.at).toISOString(),x.type,x.details.id??x.details.itemId??'',x.details.blueprint??x.details.type??'',x.details.rarity??'',x.details.dataCost??'',x.details.ingredients??'',x.details.itemEffect??'']);
  ...
  return{
    ...
    'crafting.csv':csv(['utc','event','id','type','rarity','dataCost','ingredients','itemEffect'],crafting),
